...
@Injectable({providedIn:'root'}) export class AuthService { private readonly http=inject(HttpClient); private readonly url=environment.apiUrl+API_ENDPOINTS.AUTH; private readonly key='davnorshield_token';
  /** Signs in and persists the access token. */ login(body:LoginRequest):Observable<AuthResponse>{return this.http.post<AuthResponse>(this.url+'/login',body).pipe(tap(x=>this.setToken(x.accessToken)));}
  /** Creates an account and persists the returned access token. */ register(body:RegisterRequest):Observable<AuthResponse>{return this.http.post<AuthResponse>(this.url+'/register',body).pipe(tap(x=>this.setToken(x.accessToken)));}
